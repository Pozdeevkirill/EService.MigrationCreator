    ...
    public class NewItemTarget
    {
        ...
        private static NewItemTarget CreateFromSolutionExplorerSelection(DTE2 dte)
        {
            ...
            if (items.Length == 1)
            {
                ...
                {
                    return new NewItemTarget(Path.GetDirectoryName(solution.FullName), null, null);
                }
                else if (selection.Object is Project project)
                {
                    if (project.IsKind(Constants.vsProjectKindSolutionItems))
                    {
                        return new NewItemTarget(GetSolutionFolderPath(project), project, null);
                    }
                    else
                    {
                        return new NewItemTarget(project.GetRootFolder(), project, null);
                    }
                }
        ...
        private static NewItemTarget CreateFromProjectItem(ProjectItem projectItem)
        {
            ...
            else
            {
                ...
                return new NewItemTarget(directory, projectItem.ContainingProject, projectItem);
            }
        }
        ...
        private NewItemTarget(string directory, Project project, ProjectItem projectItem)
        {
            Directory = directory;
            Project = project;
